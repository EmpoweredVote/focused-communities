...
- [06-01]: No cacheMiddleware on GET /users/:id/posts — auth-gated user-specific data; caching would serve one user's data to another
- [06-01]: !inner joins (posts→threads→communities) exclude moderated content at join time — avoids unreliable Supabase JS nested column filter syntax
- [06-01]: Header renders on all early-return paths (loading, error) in all three pages — no page state skips the shared header
